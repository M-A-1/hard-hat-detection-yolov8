...
**Training Program:** SDAIA Academy — Computer Vision for Developers with Ultralytics YOLO  
...
Training and validation losses were still decreasing at the end of the three-epoch run, while validation performance continued to improve. mAP@50 increased from 0.594 in epoch 1 to 0.632 in epoch 3, and mAP@50–95 increased from 0.366 to 0.405. Because the losses had not plateaued and there was no clear divergence between training and validation trends, the curves suggest **underfitting rather than overfitting**: the model had not yet fully converged by epoch 3. A longer training run would therefore be a logical next step before drawing conclusions about the model's maximum performance.
...
The demonstrated inference/tracking code explicitly used a **confidence threshold of 0.25**. IoU is an important NMS/localization parameter, but the documented tracking code does not explicitly set a custom IoU value, so no unsupported IoU value is reported here. The confidence threshold was kept fixed to make the demonstrated workflow reproducible.


The results show substantially stronger performance for the `head` and `helmet` classes than for the `person` class. The `person` class therefore represents an important limitation of the current model and is a clear target for future improvement.
...
These examples provide qualitative evidence that the trained model can identify helmet-related objects in images outside the training set. Individual confidence scores should not be interpreted as overall model accuracy; they indicate the model's confidence in a particular predicted object.
...
The deliverables documented in this README were executed in Google Colab and produced real captured outputs rather than code-only examples. Evidence includes:

- Actual YOLOv8 fine-tuning with `model.train()`.
- Actual validation metrics including precision, recall, mAP@50, and mAP@50–95.
- Actual image inference with detected hard hats.
- Actual instance-segmentation inference using `YOLOv8n-seg`.
- Actual real-world video processing using `model.track()`, ByteTrack, and OpenCV.
- Actual ONNX export using `model.export(format="onnx")`.

The README documents the observed results and does not present unexecuted deployment or validation claims as completed work.
...
The trained model used in the executed workflow was `best.pt`. In accordance with the repository's Git-practice guidance, trained weights should be excluded from version control via `.gitignore` rather than committed directly.
...
The repository contains the captured training/evaluation outputs (`results.csv`, `results.png`, confusion matrices, and `labels.jpg`) plus the submitted detection, segmentation, and real-world tracking evidence. Generated weights should remain excluded from version control.

The trained model was also successfully exported to ONNX during the executed workflow. The resulting `best.onnx` file is documented in the **Model Export and Deployment** section but is not currently committed to the repository.
...
The current project emphasizes image-based inference and quantitative validation. Further evaluation using a larger and more diverse independent dataset would be required before deployment in real construction-site environments.

### 5. Safety-Critical Deployment
This model is a computer-vision research/prototype system. It should not be considered a replacement for workplace safety procedures, human supervision, or certified personal protective equipment compliance.
...
The current GitHub repository contains the following submitted project files and evidence:
...
├── .gitignore
├── README.md
├── best.pt                  # trained weights used in the executed workflow; should be kept out of Git history
├── data.yaml
├── confusion_matrix.png
├── confusion_matrix_normalized.png
├── helmet_detection_result*       # detection evidence file
├── labels.jpg
├── project_summary.txt
├── real_world_hard_hat_tracking*  # real-world tracking video
├── results.csv
├── results.png
└── segmentation_result*           # instance-segmentation evidence
```

The repository was cleaned before submission by removing the temporary sample images and unnecessary development files. The trained model, evaluation outputs, detection evidence, segmentation evidence, and real-world tracking output are retained as project deliverables.

The ONNX model was successfully generated during the executed workflow, but `best.onnx` is not currently committed to the GitHub repository. The README therefore documents the ONNX export as an executed deployment/export step rather than claiming that the ONNX file is a repository file.
...
The official **SDAIA Academy GitHub** is referenced as part of the project's course/technical context:
...
The SDAIA Academy GitHub is an external course reference and is not presented as the source of the Hard Hat Workers dataset or as the author of this project.
...
Performance was particularly strong for the `head` and `helmet` classes, with mAP@50 values of **0.925** and **0.952**, respectively. In contrast, the `person` class performed poorly and represents a significant area for improvement.
